...
static var AFFIX_POOL: Array[Dictionary] = [
	...
	{ "name": "格挡率",    "type": "defense", "min": 1.0,  "max": 6.0,  "fmt": "+%.0f%%" },
	{ "name": "闪避率",    "type": "defense", "min": 1.0,  "max": 6.0,  "fmt": "+%.0f%%" },
	{ "name": "回血",      "type": "defense", "min": 1.0,  "max": 8.0,  "fmt": "+%.0f/步" },
	{ "name": "吸血",      "type": "defense", "min": 1.0,  "max": 5.0,  "fmt": "+%.0f%%" },
	{ "name": "格挡触发",  "type": "defense", "min": 5.0,  "max": 25.0, "fmt": "+%.0f%%" },
	{ "name": "闪避触发",  "type": "defense", "min": 5.0,  "max": 25.0, "fmt": "+%.0f%%" },
...
## 生成一件装备
## level: 玩家等级 (影响主属性系数)
static func generate(slot_name: String, level: int = 1) -> Dictionary:
	# 1. 品质
	...
	if slot_name in ["shoes", "ring", "necklace", "helmet"]:
		...
		# 数值类主属性受等级系数影响
		var level_coef: float = 1.0 + (level - 1) * 0.015
